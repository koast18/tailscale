...
func TestPinsSetAndGet(t *testing.T) {
	...
	got, err := e.setPins(pins.SetRequest{
		Pins: pins.Set{
			Devices:  []pins.Device{{ID: "n1"}, {ID: "n2"}},
			Services: []pins.Service{{Name: "svc:web"}},
		},
		DevicesSet:  true,
		ServicesSet: true,
	})
	...
	got, err = e.setPins(pins.SetRequest{
		Pins:       pins.Set{Devices: []pins.Device{{ID: "n3"}}},
		DevicesSet: true,
	})
	if err != nil {
		t.Fatal(err)
	...
	got, err = e.setPins(pins.SetRequest{DevicesSet: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPinsPersistAcrossStoreReopen(t *testing.T) {
	e := newTestExtension(t, "pid")
	if _, err := e.setPins(pins.SetRequest{
		Pins:       pins.Set{Devices: []pins.Device{{ID: "n1"}}},
		DevicesSet: true,
	}); err != nil {
		t.Fatal(err)
	}
...
func TestPinsNoCurrentProfileReturnsEmpty(t *testing.T) {
	...
	if !got.Equals(pins.Set{}) {
		t.Errorf("want zero pins.Set, got %#v", got)
	}
}
