...
// favorites, POST applies a [pins.SetRequest] and returns the updated set.
func servePins(h *localapi.Handler, w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		http.Error(w, "misconfigured pinning extension", http.StatusInternalServerError)
		return
	}
	var out pins.Set
	switch r.Method {
	case httpm.GET:
		var err error
		if out, err = ext.pinsFor(); err != nil {
